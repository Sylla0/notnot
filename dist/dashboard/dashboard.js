...
class NotNotDashboard {
  ...
  async init() {
    ...
    console.log('Dashboard: Notes loaded:', this.notes);
  ...
  async initializeDatabase() {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error('Dashboard: Error opening database:', request.error);
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          ...
          console.log(`Dashboard: Loaded ${this.notes.length} notes`);
          resolve();
        };
        request.onerror = () => {
          console.error('Dashboard: Error loading notes:', request.error);
          reject(request.error);
        };
      });
    } catch (error) {
      console.error('Dashboard: Error creating transaction:', error);
      this.notes = [];
      this.filteredNotes = [];
    }
  }
  ...
  setupEventListeners() {
    ...
    // Create test note button
    const testBtn = document.getElementById('create-test-note');
    if (testBtn) {
  ...
  async render() {
    ...
    console.log(`Dashboard: Rendering ${this.filteredNotes.length} notes`);
  ...
  async createTestNote() {
    console.log('Dashboard: Creating test note...');
    
    const testNote = {
      id: `note_${Date.now()}`,
      videoInfo: {
        title: 'Test Video - Getting Started with NotNot',
        url: 'https://www.youtube.com/watch?v=test123',
        platform: 'youtube',
        duration: '10:35'
      },
      notes: {
        content: '<p>This is a test note created to verify that the dashboard is working correctly.</p><p><strong>Key points:</strong></p><ul><li>NotNot helps you take notes while watching videos</li><li>You can capture screenshots at any time</li><li>Notes are automatically saved</li></ul><p>Try watching a YouTube video and pressing Alt+S to capture a screenshot!</p>'
      },
    ...
    const request = store.add(testNote);
    request.onsuccess = () => {
      console.log('Dashboard: Test note created successfully');
      // Reload notes
      this.loadNotes().then(() => this.render());
    };
    request.onerror = () => {
      console.error('Dashboard: Error creating test note:', request.error);
    };
  }
